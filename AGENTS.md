...
This is a Java 21 Maven desktop app (Swing/NetBeans forms) with a layered package layout.
...
- `src/main/java/Presentacion`: UI forms and screens (`Frm*.java` plus `.form` files).
- `src/main/java/com/mycompany/sistemavoluntariado`: application entry point.
...
- `./mvnw clean package`: compile and build the shaded JAR.
- `./mvnw exec:java`: run the desktop app (`exec.mainClass` in `pom.xml`).
...
- UI classes follow `FrmNombre` naming; services use `*Service`, DAOs use `*DAO`.
- Keep NetBeans `.form` files paired with their `.java` UI classes.
